  ...
  try {
    ...
    if (!ready) {
      await logExtension(
        'warn',
        'ready-check',
`Tab ${tab.id} not ready: composerReady=${response?.composerReady === true}, generating=${response?.generating === true}, ready=${response?.ready === true}.`,
      );
    }
  } catch (error) {
    await logExtension('warn', 'ready-check', `Unable to read ready state of tab ${tab.id}: ${errorMessage(error)}`);
    ready = false;
  }
  return {
    chatGptTabOpen: true,
...
async function prepareNewConversationTab(sourceTabId, newConversationUrl) {
  if (!sourceTabId) throw new Error('Unable to identify the current ChatCMD tab.');
  ...
  if (!tab?.id) throw new Error('Unable to open a ChatGPT tab to select a model.');
...
async function acquireNewConversationTab(sourceTabId, newConversationUrl) {
  ...
  if (!tab?.id) throw new Error('Unable to open a new ChatGPT tab automatically. Check the extension permissions and try again.');
...
async function openConversationTab(conversationUrl, sourceTabId) {
  ...
  if (!tab?.id) throw new Error('Unable to open the ChatGPT tab for this conversation.');
...
async function focusReturnSource(chatGptTabId) {
  if (!chatGptTabId) throw new Error('Unable to identify the current ChatGPT tab.');
