...
async function sendToChatGpt(tabId, payload, options = {}) {
  ...
  if (!quiet) await logExtension('info', 'background', `Sending ${payload?.type || 'message'} tới tab ${tabId}.`);
  for (let attempt = 0; attempt < 20; attempt++) {
    try {
      const response = await chrome.tabs.sendMessage(tabId, payload);
      if (response?.ok) {
        if (!quiet) await logExtension('info', 'background', `Tab ${tabId} phản hồi thành công ở attempt ${attempt + 1}.`);
        return response;
      }
      lastError = new Error(response?.error || 'ChatGPT content script could not complete the request.');
      await logExtension('error', 'content-chatgpt', `Tab ${tabId} received the request but returned: ${errorMessage(lastError)}`);
      throw lastError;
    } catch (error) {
      lastError = error;
      ...
      if (!reinjected) {
        reinjected = true;
        try {
          await logExtension('info', 'background', `Re-injecting ChatGPT content scripts into tab ${tabId}.`);
          await injectChatGptScripts(tabId);
          await logExtension('info', 'background', `Injecting ChatGPT content scripts into tab ${tabId} successfully.`);
          await delay(150);
          continue;
        } catch (injectError) {
  ...
  await logExtension('error', 'background', `Unable to send to tab ${tabId}: ${errorMessage(lastError)}`);
  throw lastError || new Error('Unable to connect to the content script on chatgpt.com.');
}
...
async function waitForTab(tabId) {
  ...
  await new Promise((resolve, reject) => {
    const timer = setTimeout(() => finish(new Error('ChatGPT page load timed out.')), 20_000);
...
async function waitForChatGptReady(tabId) {
  ...
  throw lastError || new Error('ChatGPT input box is not ready after opening the project page.');
}
...
async function handleProgress(message, tabId) {
  if (!message.requestId) throw new Error('ChatGPT progress is missing request ID.');
  const context = await requestContext(message.requestId);
  if (!context) throw new Error('ChatCMD request context not found.');
  ...
  if (message.stage === 'retrying') {
    await logExtension('warn', 'recovery', `Retrying request ${message.requestId}, attempt ${Number(message.retryCount) || 1}, reason ${message.reason || 'send_ready'}.`);
...
/**
 * Handle a single ChatGPT tool call from the content bridge.
 *
 * This is a completely independent function from handleProgress() — it does
 * NOT use the stage pipeline.  It posts directly to the dedicated tool-call
 * endpoint so the backend can route through RuntimeHost::call().
 *
 * `message` must contain: requestId, callId, tool, arguments.
 */
async function handleToolCall(message) {
  if (!message.requestId) throw new Error('chatcmd-chatgpt-tool-call thiếu requestId.');
  if (!message.callId) throw new Error('chatcmd-chatgpt-tool-call thiếu callId.');
  if (!message.tool) throw new Error('chatcmd-chatgpt-tool-call thiếu tool.');

  const context = await requestContext(message.requestId);
  if (!context) throw new Error('ChatCMD request context for tool call not found.');
  if (!context.localBaseUrl) throw new Error('ChatCMD request context is missing localBaseUrl.');

  // Tool execution can take longer than ordinary progress pings.
  const response = await fetch(`${context.localBaseUrl}/api/local/chatgpt/bridge/tools/call`, {
    method: 'POST',
  ...
  if (!response.ok) {
    let errMsg = `ChatCMD tool call trả lỗi ${response.status}.`;
    try { errMsg = (await response.json()).detail || errMsg; } catch { /* non-json */ }
    // Return ok:false with error detail so the bridge can format a result block.
    return { ok: false, callId: message.callId, tool: message.tool, error: { message: errMsg } };
  }
...
function normalizeNewConversationUrl(value) {
  ...
  if (url.origin !== 'https://chatgpt.com' || !/^\/g\/g-p-[A-Za-z0-9_-]+\/project$/.test(url.pathname) || url.search || url.hash) {
    throw new Error('ChatGPT project link has an invalid format https://chatgpt.com/g/g-p-{ID}/project.');
  }
