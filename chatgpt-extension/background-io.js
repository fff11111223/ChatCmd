...
async function sendToChatGpt(tabId, payload, options = {}) {
  ...
  for (let attempt = 0; attempt < 20; attempt++) {
    try {
      ...
      lastError = new Error(response?.error || 'ChatGPT content script không thể hoàn tất request.');
      await logExtension('error', 'content-chatgpt', `Tab ${tabId} đã nhận request nhưng trả lỗi: ${errorMessage(lastError)}`);
      throw lastError;
    } catch (error) {
      lastError = error;
...
async function handleToolCall(message) {
  ...
  const context = await requestContext(message.requestId);
  if (!context) throw new Error('Không tìm thấy ChatCMD request context cho tool call.');
  if (!context.localBaseUrl) throw new Error('ChatCMD request context thiếu localBaseUrl.');
