...
await logExtension('info', 'background', "Restored content script ${plan.kind} on tab ${tab.id} after extension reload.");
    } catch (error) {
      await logExtension('warn', 'background', `Unable to restore the ${plan.kind} content script on tab ${tab.id}: ${errorMessage(error)}`);
    }
  }
}
...
async function recoverRequestIdentity(message) {
  ...
  if (matches.length !== 1) {
    ...
    await logExtension('warn', 'recovery', `Unable to recover request ${requestId}: ${reason}; exact=${exact.length}; text=${textMatches.length}.`);
...
await logExtension('info', 'recovery', "Restored request ${requestId} on tab ${tab.id}.");
