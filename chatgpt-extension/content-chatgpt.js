...
async function runRequest(message) {
  ...
  try {
    ...
    await reportRequestResult({
      ...
      conversationId, conversationUrl: conversationUrl || window.location.href, assistantContent: currentResult,
    });
  } catch (error) {
    if (requestObservationLost(owner)) return;
    await reportRequestResult({
      ...
      conversationId, conversationUrl: conversationUrl || (started ? window.location.href : undefined),
...
async function reconcileActiveRequest(requestId) {
  ...
  if (activeRequest.observer) { await activeRequest.observer.flush(); return { reconciled: false, reason: 'browser_observer_active' }; }
  ...
  const identity = currentConversationIdentity();
  await reportRequestResult({
    requestId, status: activeRequest.stopRequested ? 'stopped' : 'completed',
    conversationId: identity?.conversationId, conversationUrl: identity?.conversationUrl || window.location.href,
...
function findComposer() {
  const direct = findUsableComposer(document, [
    ...
    'form textarea', 'form [role="textbox"]', 'form .ProseMirror[contenteditable="true"]',
    'form [contenteditable="plaintext-only"]', 'form [contenteditable="true"]',
...
function findComposerNearSendButton() {
  const sendButton = findVisible(['button[data-testid="send-button"]', 'button[aria-label="Send prompt"]', 'button[aria-label="Send message"]', 'button[aria-label*="send" i]']);
...
function findUsableComposer(root, selectors) {
  for (const sel of selectors) for (const el of root.querySelectorAll(sel)) if (isUsableComposer(el)) return el;
  return null;
}
...
function findModelSwitcherButton() {
  const selectors = ['button[data-testid="model-switcher-dropdown-button"]', 'button[aria-label*="model" i]', 'button[id*="model" i]'];
  for (const s of selectors) for (const b of document.querySelectorAll(s)) if (isVisible(b) && !b.closest('form[data-type="unified-composer"]')) return b;
  const header = document.querySelector('#page-header');
  if (!header) return null;
  return [...header.querySelectorAll('button[aria-haspopup="menu"]')].find((b) => isVisible(b) && !b.closest('form[data-type="unified-composer"]') && looksLikeModelLabel(cleanModelLabel(b.textContent || b.getAttribute('aria-label') || ''))) || null;
}

function looksLikeModelLabel(value) {
  const text = String(value || '').trim();
  return Boolean(text && /^(?:GPT(?:[-\s]?[0-9][\w.-]*)?(?:\s+(?:Pro|Thinking|Instant|Mini))?|o[1-9](?:[-\s][\w.-]+)?)$/i.test(text));
}
...
function findComposerFileInput(composer) {
  ...
  const scoped = [...(form ? form.querySelectorAll('input[type="file"]') : []), ...(composerScope && composerScope !== form ? composerScope.querySelectorAll('input[type="file"]') : [])].filter((input, index, items) => input instanceof HTMLInputElement && !input.disabled && items.indexOf(input) === index);
  const compatibleScoped = scoped.filter((input) => fileInputScore(input) > 0);
  if (compatibleScoped.length) return compatibleScoped.sort((l, r) => fileInputScore(r) - fileInputScore(l))[0];
  const explicit = [...document.querySelectorAll('input[type="file"][data-testid*="composer" i], input[type="file"][data-testid*="upload" i]')].filter((input) => input instanceof HTMLInputElement && !input.disabled && fileInputScore(input) > 0);
  return explicit.sort((l, r) => fileInputScore(r) - fileInputScore(l))[0] || null;
}

function fileInputScore(input) {
  const accept = String(input.accept || '').toLowerCase();
  return (!accept || accept.includes('text') || accept.includes('.txt') || accept.includes('*/*')) ? (3 + (input.multiple ? 1 : 0)) : 0;
}
...
function pasteFilesIntoComposer(composer, files) {
  const transfer = new DataTransfer();
  for (const file of files) transfer.items.add(file);
  composer.dispatchEvent(new ClipboardEvent('paste', { bubbles: true, cancelable: true, composed: true, clipboardData: transfer }));
}
...
function currentConversationIdentity() {
  ...
  return match ? { conversationId: decodeURIComponent(match[1]), conversationUrl: window.location.href } : null;
}

function isProvisionalConversationId(value) { return /^WEB:/i.test(String(value || '')); }

async function requestState(requestId) {
  try {
    if (!globalThis.ChatCmdRuntime.current(CONTENT_CONTEXT)) return unknownRequestState();
    const res = await globalThis.ChatCmdRuntime.sendMessage({ type: 'chatcmd-chatgpt-request-status', requestId });
    if (res?.ok !== true || res.known !== true) return unknownRequestState();
    return { known: true, running: res.running === true, stopRequested: res.stopRequested === true, hasFinalResponse: res.hasFinalResponse === true, active: res.active === true, deadlineAtMs: Number.isFinite(res.deadlineAtMs) ? res.deadlineAtMs : undefined };
  } catch { return unknownRequestState(); }
}
...
async function reportBrowserCompletion(requestId, assistantContent) {
  ...
  try {
    const response = await globalThis.ChatCmdRuntime.sendMessage({
      type: 'chatcmd-chatgpt-progress', stage: 'browser-completed', requestId,
      conversationId: identity?.conversationId, conversationUrl: identity?.conversationUrl || window.location.href, assistantContent,
    });
