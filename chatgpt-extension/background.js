...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === 'chatcmd-chatgpt-tool-call') {
    void handleToolCall(message).then((result) => sendResponse({ ok: true, ...result })).catch((error) => sendResponse({ ok: false, error: errorMessage(error) }));
    return true;
  }
...
throw new Error('Invalid fallback sub-agent request.');
  }
  ...
  if (!tab?.id) {
    throw new Error(message.conversationUrl
      ? 'Unable to reopen the current ChatGPT conversation for the sub-agent.'
      : 'Unable to open a new ChatGPT tab for the sub-agent.');
  }
...
async function reconcileOpenChatGptIdentities() {
  try {
    ...
    await logExtension('warn', 'background', `Unable to restore ChatGPT conversation identity when the extension starts: ${errorMessage(error)}`);
  }
}
  ...
  for (const [key, binding] of Object.entries(bindings)) {
    ...
    if (binding.requestId && binding.localBaseUrl) {
      try {
        ...
        await logExtension('info', 'background', `Promoted conversation ${boundId} to actual ID ${liveId}.`);
      } catch (error) {
        await logExtension('warn', 'background', `Could not synchronize the actual ChatGPT conversation ID ${liveId}: ${errorMessage(error)}`);
      }
    }
  }
