...
(() => {
  ...
  function probe(job, kind) {
    if (!isCurrent() || !ownsPage(job, kind)) throw new Error('The conversation has changed; do not send or collect content.');
    show(job);
    const marked = markedUser(job, kind);
    if (marked.duplicate) throw new Error('Multiple messages have the same compact ID; manual review is required, do not select a response automatically.');
    let text = '';
  ...
  async function prepare(job, kind, expectedToken) {
    if (expectedToken !== documentToken || !isCurrent() || !ownsPage(job, kind)) throw new Error('The tab was reloaded; restoring before sending.');
    show(job);
    ...
    if (text && !promptMatches(composer, prompt)) throw new Error('The input box contains a draft. The draft is preserved; save or clear it to continue compact.');
    if (kind === 'RESUME' && !text) await controller.selectModel(job.oldModel);
    if (!isCurrent() || !ownsPage(job, kind)) return { ready: false };
    composer = controller.findComposer();
    if (!composer || composerText(composer) !== text) throw new Error('The draft changed while preparing. Your new content is preserved.');
  ...
  async function dispatch(job, kind, expectedToken) {
    if (dispatching || expectedToken !== documentToken || !isCurrent() || !ownsPage(job, kind)) throw new Error('Cannot send: the ChatGPT document has changed.');
    ...
    const composer = controller.findComposer();
    const prompt = promptFor(job, kind, composer);
    if (!composer || !promptMatches(composer, prompt)) throw new Error('The draft changed; compact will not overwrite the content.');
